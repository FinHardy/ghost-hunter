...
def convert_all_to_boxes(stem_image_dir, output_dir, array_length, box_size, **kwargs):
    """
    Convert all images to boxes of size box_size x box_size and save them to the output_dir.

    Args:
        original_pngs (list): List of paths to the original images.
        output_dir (str): Path to the directory to save the images.
        array_length (int): Length of the 2D grid of images.
        box_size (int): Size of the box to extract.
        **kwargs: Optional keyword arguments:
            - smoothing (str): Type of smoothing to apply. Default is "sigmoid".
    ...
    sorted_image_list = logical_sort(image_list)

    for i in tqdm(range(len(sorted_image_list))):
        row = i // array_length
        col = i % array_length
        max_vals_image(
            box_size,
...
if __name__ == "__main__":
    stem_image_dir = os.path.join(
        ABS_PATH, "../../data/dm4/boracite_Diffraction_SI_png_cropped_bin1"
    )
    output_dir = os.path.join(
        ABS_PATH, "../../data/dm4/boracite_Diffraction_SI_cropped_boxed_bin1"
    )

    box_size = 5  # did a lot of experimenting and this seems to be the best size
    smoothing = "gamma"
    gamma = 1
